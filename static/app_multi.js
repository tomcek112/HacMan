...
var playerNo = document.location.href.substr(document.location.href.length - 1);


// initare our corresponding socket
/*if(playerNo == "1"){
	var socket = io.connect('http://' + document.domain + ':' + location.port + '/p1');
	var recSocket = io.connect('http://' + document.domain + ':' + location.port + '/p2');
}
else {
	var socket = io.connect('http://' + document.domain + ':' + location.port + '/p2');
	var recSocket = io.connect('http://' + document.domain + ':' + location.port + '/p1');
}*/
...
if(playerNo == "2"){
	socket.on('p1 loc down', function(msg) {
			console.log("p1 loc updated");
		    player.x = msg['x'];
		    player.y = msg['y'];
		});
}
else {
	socket.on('p2 loc down', function(msg) {
				console.log("p2 loc updated");
		        ghostPink.x = msg['x'];
		        ghostPink.y = msg['y'];
		});
}	
...
var draw = function() {
	...
	if(playerNo == 1){
		...
		socket.emit('p1 loc', {x: player.x, y: player.y});
	}
	else {
		...
		player.eat();
		socket.emit('p2 loc', {x: ghostPink.x, y: ghostPink.y});	
	}
